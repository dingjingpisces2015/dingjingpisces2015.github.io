...
//  puzzle
//
//  Created by 丁京 on 2019/6/9.
//  Copyright © 2019年 com.dingjing.coolest. All rights reserved.
//
class Solution
{
    var founded = false
    ...
    func run(_ puzzle:[[Int]]) -> Int {
        let ret = forceSearch(puzzle, [Int]())
        return ret
    }
    
    func forceSearch(_ puzzle:[[Int]], _ markedQueue:[Int]) -> Int {
        if isFound(puzzle) == true {
            return 0
        } else {
            var queue = markedQueue
            queue.append(sequenceArray(puzzle)!)
            var minResult = -100
            for round in 0..<4 {
                let (movedPuzzle, success) = move(puzzle, round)
                if queue.contains(sequenceArray(movedPuzzle)!) {
                    continue
                }
                if (success) {
                    let result = forceSearch(movedPuzzle, queue)
                    if minResult == -100 {
                        minResult = result
                    }
